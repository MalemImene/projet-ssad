...
class Stegno:
    ...
    def main(self, root):
        ...
        title.config(font=('sans-serif', 30))
        title.grid(pady=10)

        b_encode = Button(f, text="Encode",
                          command=lambda: self.frame1_encode(f), padx=14, bg="#00FF77")
        b_encode.config(font=('sans-serif', 18))
        b_decode = Button(f, text="Decode", padx=14,
                          command=lambda: self.frame1_decode(f), bg="#FF4400")
        b_decode.config(font=('sans-serif', 18))
...
#https://github.com/praveenvarma666/Image-Steganograpgy